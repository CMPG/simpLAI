...
const float invLog2=1.0f/ 0.69314718055995f; 
...
void fill_icsi_log_table2(const unsigned precision, float* const   pTable)
{
   ...
   int size = 1 << precision;
   float invSizef=1.0f/(float)( 1 << precision );
   for(int i = 0;  i < size;  ++i )
   {
      // make y-axis value for table element
      pTable[i] = float(log(oneToTwo)) * invLog2;
      oneToTwo += invSizef;
    }
}
//
///* ICSIlog V 2.0 */
//void fill_icsi_log_table2_old(const unsigned precision, float* const   pTable)
//{
//    /* step along table elements and x-axis positions
//      (start with extra half increment, so the steps intersect at their midpoints.) */
//    float oneToTwo = 1.0f + (1.0f / (float)( 1 <<(precision + 1) ));
...
//        // make y-axis value for table element
//        pTable[i] = float(log(oneToTwo)) / 0.69314718055995f;
//        oneToTwo += 1.0f / (float)( 1 << precision );
//    }
//}
//
////Loro_27_03_16 New multithreaded version
//void fill_icsi_log_table2_multiT(const unsigned precision, float* const   pTable)
//{
//   /* step along table elements and x-axis positions
//     (start with extra half increment, so the steps intersect at their midpoints.) */
//   float oneToTwo = 1.0f + (1.0f / (float)( 1 <<(precision + 1) ));

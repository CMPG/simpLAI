...
const double _min_float_ = -std::numeric_limits<float>::min();
...
/* ICSIlog v2.0 */
inline float icsi_log_v2_23(const float val, register float* const pTable)
{
     /* get access to float bits */
     register const int* const pVal = (const int*)(&val);
     ...
     /* extract exponent and mantissa (quantized) */
     register const int exp = ((*pVal >> 23) & 255) - 127;
     register const int man = (*pVal & 0x7FFFFF);

...
using namespace std;
extern int RETURN_PROBLEM;

//Loro_06_10_12  #include "AFS.h" necessary here to prevent copilatin errors with Borland C++Builder
#include "anyoption.h"
//#include "AFS.h"
#ifdef _GCC_
#include <iostream>
...
//Loro_13_02_10 added header file for strcomp
...
AnyOption::init(int maxopt, int maxcharopt) {
   ...
   if (alloc() == false) {
      std::cerr << endl << "OPTIONS ERROR : Failed allocating memory\n" << "Exiting." << endl;
      exit(RETURN_PROBLEM);
   }
}
